...
export default function SessionItem({ session, isActive, onSelect, onDelete }: SessionItemProps) {
  const displayName = session.displayName || session.name.split('/').pop() || 'Untitled';
    ...
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-4 w-4 shrink-0 opacity-50">
      ...
      <button
        ...
        className="ml-1 hidden shrink-0 rounded-md p-1 text-gray-500 hover:bg-white/10 hover:text-red-400 group-hover:block"
